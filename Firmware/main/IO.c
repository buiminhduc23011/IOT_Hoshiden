...
void IO_Init()
{
    ...
    io_conf.pin_bit_mask = (1ULL << SS_DETECT) | (1ULL << INPUT_SPARE_1) | (1ULL << INPUT_SPARE_2) | (1ULL << INPUT_SPARE_3); // Các chân input
    ...
    io_conf.pin_bit_mask = (1ULL << XL01) | (1ULL << RED) | (1ULL << YEL) | (1ULL << GRE) | (1ULL << BUZZ) | (1ULL << TRIGGER_CAMERA); // Các chân output
    io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;                                                                                      // Không sử dụng pull-down
    io_conf.pull_up_en = GPIO_PULLUP_DISABLE;                                                                                          // Không sử dụng pull-up
    gpio_config(&io_conf);
...
void SetStateXL(uint16_t state)
{
    State_XL = state;
}
void E_interval()
...
void io_task(void *pvParameter)
{
    while (1)
    {
        ...
        if (CheckCam)
        {
            if (TimeoutCamera < 600)
            {
                TimeoutCamera++;
            }
            if (TimeoutCamera == 500)
            {
